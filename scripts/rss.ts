...
const main = () => {
  const feed = new RSS({
    title: 'Arvin Singh',
    site_url: 'https://apsbali.com',
    feed_url: 'https://apsbali.com/feed.xml',
    // image_url: 'https://apsbali.com/og.png',
    language: 'en',
    description: "Arvin Singh's blog",
  })

  combined.forEach((post) => {
    const url = `https://apsbali.com/${post.type === 'post' ? 'blog' : 'notes'}/${post.slug}`
    feed.item({
      title: post.title,
