...
export const getRecentContent = async (limit = 10) => {
  ...
  const postsWithType = posts.map((post: any) => ({ ...post, contentType: 'blog' as const }))
  const notesWithType = notes.slice(0, limit).map((note: any) => ({ ...note, contentType: 'note' as const }))
...
export const searchContent = async (query: string) => {
  ...
  return {
    ...
    notes: filteredNotes,
    total: filteredPosts.length + filteredProjects.length + filteredNotes.length
  }
}
